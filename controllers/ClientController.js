...
const getClients = async (req, res, next) => {
  ...
  const clients = await clientCollection.find({ archived: {$in: [false, null] }}).toArray();
...
const getCurrentClients = async (req, res, next) => {
  const conversationCollection = mongoClient.db('teamregex').collection('conversations');
